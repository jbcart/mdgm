...
graph_from_nug <- function(nugmat) {
  ...
  el <- Reduce(rbind, lapply(1:length(nbh_partial), function(i) {
    if (length(nbh_partial[[i]]) == 0) {
      matrix(, nrow = 0, ncol = 2)
    } else {
      cbind(nbh_partial[[i]], i)
    }
  }))
...
generate_rdag <- function(nug, weights, root) {
  ...
  while (!all(labeled)) {
    ...
    df <- data.frame(
      v = Reduce(c, connections),
      d = Reduce(c, weights[next_vertices]) +
        rep(labels[next_vertices],
          times = sapply(connections, length)
        )
    )
    df <- df[!labeled[df$v], ]
...
discrete_hmrf_mcmc <- function(
    y, nug, inits, psi_tune, M = 100, J = 1000, a = 1, b = 1,
