...
In a hierarchical model, the spatial field $z$ is latent and observations $y$
are generated through an **emission distribution**. Both MDGM and MRF spatial
models support the same three emission families:
...
```{r bernoulli-sim, fig.width = 5, fig.height = 4}
set.seed(1)
p_true <- c(0.3, 0.7)
...
```{r bernoulli-psi-trace, fig.width = 6, fig.height = 3}
...
```{r gaussian-psi-trace, fig.width = 6, fig.height = 3}
...
```{r poisson-psi-trace, fig.width = 6, fig.height = 3}
